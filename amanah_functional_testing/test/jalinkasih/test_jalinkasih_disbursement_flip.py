# Generated by Selenium IDE
import pytest
import time
import json
from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.support import expected_conditions
from selenium.webdriver.support.wait import WebDriverWait
from selenium.webdriver.common.keys import Keys
from selenium.webdriver.common.desired_capabilities import DesiredCapabilities

from selenium.webdriver.support import expected_conditions as EC

from utils.assert_attribute import assert_id_exists, assert_unique_attribute_exists
from utils.assert_redirect import assert_url_contains
from utils.webdriver_setup import get_webdriver

class TestDisbursementFlip():
  def setup_method(self, method):
    self.driver = get_webdriver()
    self.vars = {}
  
  def teardown_method(self, method):
    self.driver.quit()
  
  def test_DisbursementFlip(self):
    self.driver.execute_cdp_cmd("Page.addScriptToEvaluateOnNewDocument", {
    "source": """
        localStorage.setItem('appearance-config', '{"state":{"interfaceKit":"donor","colorTheme":"pacific"},"version":0}');
    """
    })
    self.driver.get("https://jalinkasih.amanah-staging.cs.ui.ac.id/")
  
    self.driver.refresh()
    time.sleep(3)

    donasi_button = WebDriverWait(self.driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//li[contains(@class, 'dropdown')]//button[span[text()='Program']]"))
    )
    donasi_button.click()

    # Wait for the "Aktivitas" link to be visible and click it
    activity_link = WebDriverWait(self.driver, 10).until(
        EC.visibility_of_element_located((By.XPATH, "//ul[contains(@class, 'dropdown-content')]//a[@href='/activity']"))
    )
    activity_link.click()

    time.sleep(2)

    detail_button = WebDriverWait(self.driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//a[@href='/activity/0b2d6b0a-13f7-4f62-945b-15840cf2d541']/button[text()='Detail']"))
    )
    detail_button.click()

    time.sleep(2)

    withdraw_button = WebDriverWait(self.driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[text()='withdraw']"))
    )
    withdraw_button.click()

    time.sleep(2)


    # Form data
    self.driver.find_element(By.NAME, "account_number").click()
    time.sleep(1)
    self.driver.find_element(By.NAME, "account_number").send_keys("8112830282")
    time.sleep(1)

    self.driver.find_element(By.NAME, "amount").click()
    time.sleep(1)
    self.driver.find_element(By.NAME, "amount").send_keys("123456")
    time.sleep(1)

    self.driver.find_element(By.NAME, "bank_code").click()
    time.sleep(1)
    self.driver.find_element(By.NAME, "bank_code").send_keys("BCA")
    time.sleep(1)

    self.driver.find_element(By.NAME, "name").click()
    time.sleep(1)
    self.driver.find_element(By.NAME, "name").send_keys("selenium user")
    time.sleep(1)
 
    self.driver.find_element(By.NAME, "phone").click()
    time.sleep(1)
    self.driver.find_element(By.NAME, "phone").send_keys("089517313109")
    time.sleep(1)

    self.driver.find_element(By.NAME, "email").click()
    time.sleep(1)
    self.driver.find_element(By.NAME, "email").send_keys("try@gmail.com")
    time.sleep(1)

    self.driver.find_element(By.XPATH, "//textarea[@name='description']").click()
    time.sleep(1)
    self.driver.find_element(By.XPATH, "//textarea[@name='description']").send_keys("withdraw selenium")
    time.sleep(1)

    self.driver.find_element(By.XPATH, "//input[@type='date']").click()
    time.sleep(1)
    self.driver.find_element(By.XPATH, "//input[@type='date']").send_keys("2024-12-15") 
    time.sleep(1)

    self.driver.find_element(By.NAME, "vendor_name").click()
    time.sleep(1)
    dropdown = self.driver.find_element(By.NAME, "vendor_name")
    dropdown.find_element(By.XPATH, "//option[. = 'Flip']").click()
    time.sleep(1)

    self.driver.find_element(By.CSS_SELECTOR, ".card-actions > .btn").click()
    time.sleep(2)

    self.driver.find_element(By.NAME, "disbursementMethod").click()
    time.sleep(2)

    dropdown = self.driver.find_element(By.NAME, "disbursementMethod")
    dropdown.find_element(By.XPATH, "//option[. = 'disbursement']").click()
    time.sleep(2)

    self.driver.find_element(By.CSS_SELECTOR, ".card-actions > .btn").click()
    time.sleep(2)

    element = self.driver.find_element(By.CSS_SELECTOR, ".card-actions > .btn")
    time.sleep(1)

    
    # Assert page redirect
    expected_url = "https://jalinkasih.amanah-staging.cs.ui.ac.id/viapaymentgateway/list"
    assert_url_contains(self.driver, expected_url)
    
    # Navigate to the last page if pagination exists
    while True:
        try:
            next_button = self.driver.find_element(By.XPATH, "//button[span[@class='sr-only' and text()='Last']]")
            if not next_button.is_enabled():
                break
            next_button.click()
            time.sleep(2)
        except:
            break

    # Click the last item in the table
    time.sleep(2)
    table = self.driver.find_element(By.CSS_SELECTOR, ".table tbody")
    rows = table.find_elements(By.TAG_NAME, "tr")
    if rows:
        last_row = rows[-1]
        detail_button = last_row.find_element(By.CSS_SELECTOR, ".btn")
        detail_button.click()
        time.sleep(2)
        
        assert_unique_attribute_exists(self.driver, "paymentlink")
        assert_id_exists(self.driver)